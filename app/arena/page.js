...
export default function ArenaPage() {
  ...
  // useLayoutEffect: runs synchronously after DOM paint — canvas dimensions guaranteed correct
  useEffect(() => {
    ...
    // Get exact container size from DOM
    ...
    function spawn(x, y, color, n) {
      for (let i = 0; i < (n||12); i++) gs.particles.push(makeParticle(x, y, color));
    }
    function floatNum(x, y, dmg, color) {
    ...
    function attack(att, def, move) {
      ...
      if (move === 'block') {
        att.state = 'block';
        setTimeout(() => { if (att.state === 'block') att.state = 'idle'; }, 480);
        return;
      }
      ...
      floatNum(ax, H * 0.45, dmg, move === 'special' ? '#fbbf24' : att.char.color);
      setTimeout(() => { if (att.state === 'attack') att.state = 'idle'; },
        move === 'special' ? 700 : move === 'kick' ? 420 : 280);
    }
    ...
    window._dominexAttack = (player, move) =>
      attack(player === 1 ? gs.p1 : gs.p2, player === 1 ? gs.p2 : gs.p1, move);

    function drawArena() {
      const sky = ctx.createLinearGradient(0, 0, 0, H);
      sky.addColorStop(0, '#14003a');
      sky.addColorStop(0.6, '#2a0828');
      sky.addColorStop(1, '#3a0a08');
      ctx.fillStyle = sky; ctx.fillRect(0, 0, W, H);
      // Stars
      for (let i = 0; i < 80; i++) {
        const r = i % 5 === 0 ? 2.0 : 1.2;
        ctx.fillStyle = 'rgba(255,255,255,0.8)';
        ctx.beginPath();
        ctx.arc((i * 137 + 43) % W, (i * 97 + 17) % (H * 0.6), r, 0, Math.PI * 2);
        ctx.fill();
      }
      // Ground
      ...
      [W * 0.08, W * 0.92].forEach(cx => {
        ...
        fire.addColorStop(0, '#fff7aa'); fire.addColorStop(0.4, '#f97316');
        fire.addColorStop(1, 'rgba(239,68,68,0)');
        ctx.fillStyle = fire;
        ctx.beginPath();
        ctx.arc(cx, H * 0.28 + Math.sin(frame * 0.15) * 4, 26, 0, Math.PI * 2);
        ctx.fill();
      });
    }
    ...
    function drawHUD() {
      ...
      ctx.strokeStyle = 'rgba(255,255,255,0.2)'; ctx.lineWidth = 1; ctx.strokeRect(p2bar, M, BAR, BH);
    ...
    function loop() {
      ...
      if (!gs.over && (gs.p1.hp <= 0 || gs.p2.hp <= 0 || gs.time <= 0)) {
        ...
        const fi = setInterval(() => {
          ...
          ctx.fillRect(0, 0, W, H); fl++; if (fl > 5) clearInterval(fi);
        }, 80);
        setTimeout(() => {
          ...
          const winName = w === 'P1' ? gs.p1.char.name : w === 'P2' ? gs.p2.char.name : '';
          ctx.fillText(w === 'DRAW' ? 'DRAW!' : winName + ' WINS!', W / 2, H / 2 + 45);
          ctx.font = '18px Inter,sans-serif'; ctx.fillStyle = '#f59e0b';
          ctx.fillText(
            w === 'P1' ? '+' + betAmount + ' $DMX earned' :
            w === 'P2' ? '-' + betAmount + ' $DMX lost' : '',
            W / 2, H / 2 + 85
          );
    ...
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#030308,#0a0005)', color:'white', fontFamily:'Inter,sans-serif', display:'flex', flexDirection:'column', alignItems:'center', padding:'28px 16px' }}>
      ...
      {p1Char && step === 2 && (
        <div style={{ marginBottom:16, fontSize:14, color:'#94a3b8' }}>
          P1: <span style={{ color:p1Char.color, fontWeight:800 }}>{p1Char.name}</span> &nbsp;vs&nbsp; <span style={{ color:'#64748b' }}>???</span>
        </div>
      )}
