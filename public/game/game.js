...
function drawFighter(ctx,f,t){
  ...
  // ROT_SPRITES only in char select (not fight) — fight uses normal sprite + dir flip
  if(st==='idle' && rotF && G.screen!=='fight'){
...
function initFight(){
  ...
  function waitAndInit(attempts) {
    ...
    G.gs={
      p1:{ch:G.player,x:W*0.28,y:FLOOR,vy:0,onGround:true,hp:G.player.hp*2,maxHp:G.player.hp*2,energy:0,state:'idle',af:0,cd:0,dir:1,H:Math.round(SC*130),dmgTaken:0},
      p2:{ch:opp,x:W*0.72,y:FLOOR,vy:0,onGround:true,hp:Math.round(opp.hp*(G.mpMode?2:eHpMult*2)),maxHp:Math.round(opp.hp*(G.mpMode?2:eHpMult*2)),energy:0,state:'idle',af:0,cd:0,dir:-1,H:Math.round(SC*130),dmgTaken:0},
      timer:90,lastSec:Date.now(),
      p1r:0,p2r:0,round:1,
...
function cpuThink(gs){
  // Skip CPU AI in multiplayer mode (check both G.mpMode and live MP.active flag)
  if(G.mpMode || (window.MP && window.MP.active)) return;
  ...
  if(gs.phase==='roundOver'||gs.phase==='matchOver'){
    ...
    if(gs.phase==='roundOver'&&gs.roundOverTimer>=90){
      if(gs.p1r>=2||gs.p2r>=2){
        ...
        gs.p1.cd=gs.p2.cd=0;gs.timer=90;gs.lastSec=Date.now();gs.round++;
...
function showResult(win,gs){
  ...
  // ── Set up navigation buttons FIRST ──
  ...
  try{
    ...
    if(rs){
      rs.style.setProperty('--res-c1',c1);
      rs.style.setProperty('--res-glow',col);
    }

    // Title + sub
    var rt=$('res-title');
    if(rt){
      rt.textContent=champion?'CHAMPION!':win?'YOU WIN!':'DEFEAT!';
      rt.style.color=col;
    }
    var rsub=$('res-sub');
    ...
    try{if(win)announce(champion?'Champion!':'You Win!',200);else announce('Defeat!',200);}catch(e){}
  }catch(e){}
  ...
  try{showScreen('result');}catch(e){
    ...
    var _rs2=document.getElementById('result-screen');if(_rs2)_rs2.classList.add('active');
  }
}
...
window.showMPResultScreen = function(won) {
  ...
  // MENU button — disconnect and go to main menu
  ...
  menu.ontouchend = menu.onclick = function(e){
    e.preventDefault();e.stopPropagation();
    // Disconnect socket
    if(window.MP && window.MP.socket) {
      try { window.MP.socket.disconnect(); } catch(ex){}
    }
